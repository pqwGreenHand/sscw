        ...
        columns: [[
            ...
            },
            {
                field: 'status', title: '状态', width: 80, formatter: function (value, row, index) {
                    var str = "";
                    if(value==1)
                        str = "<font color='green'>审讯中</font>";
                    if(value==3)
                        str = "<font color='gray'>已完成</font>";
                    if(value==0)
                        str = "<font color='blue'>初始化</font>";
                    return str;
                }
            },
            {
            ...
            //     }},
            {
                ...
                formatter: function (value, row, index) {
                    ...
                    if(row.status==3){
                    	if (isGridButton("recordLoadTd")) {
                        f ='<span class="spanRow" ><a href="#" class="gridseeinfo" onclick="recordInfo('+index+')">查看详情</a></span>';
                    	}
                    	if (isGridButton("recordLoad")) {
                    		f ='<span class="spanRow" ><a href="#" class="gridseeinfo" onclick="load('+index+')">下载</a></span>';
                    	}
                    }
                    return e + d + f;
                }
...
function selectRecordType(recordType){
    ...
    jQuery.ajax({
        ...
        success : function(data){
            if(!data.error){
                flageTypeData = data.callbackData.flageTypeData.content;
    ...
    $('#flageType').combobox("loadData", flageTypeData);
