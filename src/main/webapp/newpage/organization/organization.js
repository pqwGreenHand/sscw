$(function(){
	$('#datagrid').datagrid({
		...
		loadMsg: 'Loading...',
		method: 'get',
		url: '/sscw/zhfz/common/organization/list.do',
		queryParams : {
...
//			$('#datagrid').datagrid('reload',{trefresh:new Date().getTime()} );
	...
	$('#cdatagird').datagrid({
		...
		loadMsg: 'Loading...',
		method: 'get',
		url: '/sscw/zhfz/common/organization/listChildOrg.do',
		queryParams : {
			trefresh:new Date().getTime()
		},
		sortName: 'id',
...
function organizationAdd(){
	...
	$('#parentId').combobox({   
	    url: contextPath + '/sscw/zhfz/common/combobox/listAllOrganizationNameCombobox.do',
	    valueField:'id',   
	    textField:'value',
        required: true,
	...
	url = '/sscw/zhfz/common/organization/add.do';
}
...
function organizationEdit(index){
	...
	if (row){
		...
		$('#organization_form').form('clear');
		$('#parentId').combobox({   
			url:'/sscw/zhfz/common/organization/upOrgCombo.do',
			valueField:'id',   
			textField:'name',
			required: true,
			filter: function(q, row){
	    ...
	    url = '/sscw/zhfz/common/organization/edit.do';
	} else{
        ...
        $.messager.confirm('确认','是否要删除数据?',function(r){  
            if (r){
            	...
            	jQuery.ajax({
        			...
        			type : 'POST',
        			contentType : 'application/json',
        			url : '/sscw/zhfz/common/organization/remove.do',
        			data : '{"id":'+value+'}',
        			dataType : 'json',
        			success : function(data){
        				$.messager.alert(data.title, data.content); 
...
function refrshAuthority() {
    $.ajax({
        //async: false,
        type: 'POST',
        url: '/sscw/zhfz/common/authority/refrshAuthority.do',
        dataType: 'json',
        success: function (data) {
